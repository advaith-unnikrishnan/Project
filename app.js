...
var serverUrl =
  "https://6ed2de18-f11d-4424-83cd-5f9fefb58d6d.mock.pstmn.io/disasterdata";
  ...
  Locations.forEach((location) => {
    if (location.X == lat.value && location.Y == lon.value) {
      if (location.status === "High Risk")
        locationOutput.innerText = `The location is in ${
          location.status
        } zone and it has a ${
          location.target_2019 * 100
        }% chance of flood occurence`;
        ...
        locationOutput.innerText = `The location has ${
          location.target_2019 * 100
        }% chance of flood occurence, you can ${location.status}`;
      else if (location.status === "Be on alert")
        locationOutput.innerText = `The location has  ${
          location.target_2019 * 100
        }% chance of flood occurence. ${location.status}`;
      else
        locationOutput.innerText = `The location is in ${
          location.status
        } and it has a ${
